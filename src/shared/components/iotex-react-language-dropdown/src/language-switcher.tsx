...
const LanguageSwitcher = (props: Props) => {
  ...
  const menuListElement = useRef<HTMLUListElement>(null);
  ...
  const initLanguageMenu = (props: Props): Array<languageMenu> => {
  ...
  const toggleTranslationMenu = (tState?: boolean) => (
    event: React.MouseEvent<HTMLButtonElement, MouseEvent>
  ) => {
    event.persist();
    const isDisplay =
      typeof tState === "boolean" ? tState : !state.displayTranslationMenu;
    ...
    if (isDisplay) {
      ...
      const ele: HTMLUListElement = menuListElement.current as HTMLUListElement;
   ...
   */
  const canRun = (): Promise<boolean> => {
    return new Promise(resolve => {
      setTimeout(() => resolve(noClickHappen), 100);
    });
  };
  ...
  const { style = { width: "3em", display: "inline-block" } } = props;
  const languages = initLanguageMenu(props);

  const translationBlock = (
    <div
      style={{
        display: state.displayTranslationMenu ? "block" : "none"
      }}
    >
      <LanguageMenu ref={menuListElement}>
        {languages.map((o, i) => {
          ...
          return (
            <LanguageItem key={i}>
              <LAnchor
                ...
                onMouseOver={() =>
                  setState(prevState => ({ ...prevState, linkCIndex: i }))
                }
                onMouseLeave={() => {
                  setState(prevState => ({ ...prevState, linkCIndex: -1 }));
                }}
